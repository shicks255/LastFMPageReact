...
const Pagination: React.FC<IProps> = (props: IProps): JSX.Element => {
  ...
  const firstLink = (
    <Link to={replacePageParam(1)}>
      <button type="button" className="p-2 bg-blue-200">
        1
      </button>
    </Link>
  );
    ...
    <div>
      ...
      <span className="">
        <span className="">
          ...
          {firstPage || secondPage ? '' : preLink}
          <button type="button" className="p-3 bg-blue-400 border-solid">
            {page}
          </button>
          {lastPage ? '' : postLink}
          ...
          {penultimatePage || lastPage ? '' : lastLink}
        </span>
        <span>
          <span className="ml-2 mr-2" />
          {lastPage ? (
